...
% See upper panels in figures 4 and 5
...
mu = 0.5;
r = lambda-mu;
q = 0; % q=0 for figure 4 and q=1 for figure 5
k = 150; 
n = 1:k;
...
for n0 = [40,50,60,70] % the values for the fixed point population size
    ...
    g = r-n0*lambda/k;
    h0 = 1/(n0*g); % see eq.(12)
    h1 = lambda*k*((1-q)*lambda*n0+q*r*k)/(n0*(1+q)*(g*k)^2); % see eq.(11)
    hv = 0:0.0005:min([h0,h1,0.06]);
